...
EGG_TARGET = 20
HERO_START = (200, 300)
ATTACK_DISTANCE = 150  # Adjusted the attack distance
DRAGON_WAKE_TIME = 2
EGG_HIDE_TIME = 2
MOVE_DISTANCE = 5
...
def check_for_dragon_collision(lair):
    # Calculate the distance between hero and dragon in both X and Y directions
    x_distance = abs(hero.x - lair["dragon"].x)
    y_distance = abs(hero.y - lair["dragon"].y)

    # Check if hero is within the dragon's attack range in both X and Y directions
    if x_distance < ATTACK_DISTANCE and y_distance < ATTACK_DISTANCE:
        handle_dragon_collision()

def handle_dragon_collision():
    global reset_required
    reset_required = True
    animate(HERO_START, on_finished=subtract_life)

def check_for_egg_collision(lair):
    global eggs_collected, game_complete
    if hero.colliderect(lair["eggs"]) and not lair["egg_hidden"]:
...
def subtract_life():
    ...
    lives -= 1
    reset_required = False  # Reset the flag to allow for further life deductions
    if lives == 0:
        game_over = True
